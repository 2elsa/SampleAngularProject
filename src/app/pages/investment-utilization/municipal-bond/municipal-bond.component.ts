...
import { UserMunicipalBond, FlxWealthManagerApiClient } from '@services/flxwealthmanager.api.client';
...
export class MunicipalBondComponent extends UserSelectionComponentHelper<UserMunicipalBond> implements OnInit {
  
  title = 'Edit Municipal Bond';
  ...
  ngOnInit(): void {
    this.init(
      [this.investmentUtlizationState$.pipe(map(m => m.municipalBondOptions)),
      this.investmentUtlizationState$.pipe(map(s => s.userMunicipalBonds))], investmentUtilizationOptionIdMapper.municipal_Bond, {
      fromJsConverter: UserMunicipalBond.fromJS,
      onUserSelected: (option: ExtendedNamedModel, model: UserMunicipalBond) => {
        return this.store.dispatch(new InvestmentUtilizationActions.AddedUserMunicipalBond(option, model));
      },
      onUserDeleted: (model: ExtendedNamedModel) => {
        return this.store.dispatch(new InvestmentUtilizationActions.DeletedUserMunicipalBond(model))
      }
    });
  }
}
